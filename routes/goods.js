...
router.get('/getIndexInfo',(req,res)=>{
    connection.query('select goodsname,saleNum from goodstable order by saleNum desc limit 0,10',(err,result)=>{
        if(err) throw err;
        let goodsList1 = result;
        connection.query('select goodsname,stockNum from goodstable order by stockNum asc limit 0,10',(err,result)=>{
            if(err) throw err;
